// https://minecraft.fandom.com/wiki/Commands/title

import { TARGET } from "../arg/selector";
import { raw } from './raw'
import { NBTText } from "../arg/nbt";

export function title(target: TARGET) {
    return {
        clear: () => raw(`title ${target} clear`),
        reset: () => raw(`title ${target} reset`),
        times: (fadeIn: number, stay: number, fadeOut: number) => raw(`title ${target} times ${fadeIn} ${stay} ${fadeOut}`),
        title: (text: NBTText) => raw(`title ${target} title ${text}`),
        subtitle: (text: NBTText) => raw(`title ${target} subtitle ${text}`),
        actionbar: (text: NBTText) => raw(`title ${target} actionbar ${text}`),
    }
}
